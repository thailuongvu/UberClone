...
function ProfileScreen({ navigation }) {
    ...
    return (
      ...
      <View className="flex-1">
                    ...
                    renderItem={({item})=>(<TouchableOpacity>
                        <View className='p-2 pl-6 pb-2 pt-2 bg-gray-300 m-2 flex-row justify-between'>
                            <View className='flex-row'>
                                <View className='mt-1'>
                                    {item.id=='789'||item.id=='777'?<Ionicons name={item.icon} size={24} color="black" />
        ...
        <Button
                style={{marginTop: 20, width: 380, height: 50, backgroundColor: '#333', marginLeft: 8, borderRadius: 10}}
                title='LOGOUT'
                onPressFunction={logoutFunction}
                color='#fff'
            />
      </View>
    );
